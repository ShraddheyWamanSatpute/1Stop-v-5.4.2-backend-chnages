...
const BASE_PROVIDER_STORAGE_KEY = "onestop:admin-data-provider"
const OVERRIDE_STORAGE_KEY = "onestop:admin-data-provider-overrides"
...
const parseOverrideString = (raw: string): Partial<DataProviderMap> => {
  ...
  for (const pair of trimmed.split(",")) {
    ...
    overrides[moduleName] = parseProvider(providerRaw, "firebase")
  }
...
export const getBaseDataProvider = (): DataProviderName => {
  const envProvider = parseProvider(env.VITE_ADMIN_DATA_PROVIDER || env.VITE_DATA_PROVIDER, "firebase")
  return parseProvider(readStorageValue(BASE_PROVIDER_STORAGE_KEY), envProvider)
}

export const getDataProviderOverrides = (): Partial<DataProviderMap> => {
  const envOverrides = parseOverrideString(
    String(env.VITE_ADMIN_DATA_PROVIDER_OVERRIDES || env.VITE_DATA_PROVIDER_OVERRIDES || ""),
  )
