...
// Vite config for building Main Site section (marketing site)
export default defineConfig({
  ...
  resolve: {
    dedupe: ["firebase", "@firebase/app", "@firebase/component", "@firebase/util"],
    alias: [
      // YourStop @/ alias
      {
        find: /^@\/(.*)$/,
  ...
  build: {
    ...
    rollupOptions: {
      input: {
        'main-site': path.resolve(__dirname, 'index.html'),
      },
      onwarn(warning, warn) {
  ...
  define: {
    'process.env.BUILD_SECTION': JSON.stringify('main-site'),
  },
})
