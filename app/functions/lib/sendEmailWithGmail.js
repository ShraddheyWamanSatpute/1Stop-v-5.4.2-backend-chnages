...
exports.sendEmailWithGmail = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    ...
    try {
        ...
        const emailConfig = await (0, mailConfigSecrets_1.loadMailboxConfig)(`companies/${companyId}/sites/${siteId || 'default'}/subsites/${subsiteId || 'default'}`, configPath, 'bookings');
        if (!emailConfig || !emailConfig.email || !emailConfig.appPassword) {
            res.status(404).json({
                success: false,
                error: 'Email not configured. Please configure your Gmail mailbox and ensure the password is stored server-side.'
            });
            return;
        }
