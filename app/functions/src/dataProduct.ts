...
const getSupabaseConfig = () => {
  ...
  if (!url || !serviceRoleKey) {
    throw Object.assign(
      new Error("Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY for Product provider"),
      { status: 500 },
    )
  }
...
const createRow = (scope: Scope, id: string, payload: any) => {
  ...
  return {
    ...
    code: cleaned?.sku || cleaned?.barcode || null,
...
const upsertEntity = async (table: string, scope: Scope, id: string, payload: any) => {
...
export const handleProductDataRequest = async ({ req, res, path, body, user }: HandlerArgs) => {
  ...
  if (listMatch && method === "GET") {
    ...
    const table = entityMap[listMatch[1] as keyof typeof entityMap]
  ...
  if (listMatch && method === "POST") {
    ...
    const row = await upsertEntity(table, scope, id, body?.data || {})
  ...
  if (itemMatch && method === "PATCH") {
    const scope = parseScope(String(body?.basePath || ""))
    await assertCompanyAccess(user.uid, scope.companyId)
    const table = entityMap[itemMatch[1] as keyof typeof entityMap]
    const id = decodeURIComponent(itemMatch[2])
    const row = await upsertEntity(table, scope, id, body?.updates || {})
